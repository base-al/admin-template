      ...
      <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row gap-6 items-start sm:items-center justify-between">
          <div class="flex items-center gap-4">
            ...
            />
            <div class="space-y-1">
              <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">Tag Details</h1>
              <p class="text-sm text-gray-600 dark:text-gray-400">View tag information</p>
            </div>
          </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      <UCard>
        ...
        <div class="space-y-4">
          ...
          <div>
            <label class="text-sm text-gray-600 dark:text-gray-400">Slug</label>
            <p class="text-base font-medium">{{ item.slug }}</p>
          </div>
          <div>
  ...
  <UDashboardPanel v-else>
    <template #body>
      <div class="flex flex-col items-center justify-center py-12 gap-3">
        ...
        <p class="text-lg text-gray-500 dark:text-gray-400">Tag not found</p>
...
const handleEdit = () => {
  ...
  router.push(`/app/tags/${id.value}/edit`)
}
...
const confirmDelete = async () => {
  deleting.value = true
  try {
    await tagsStore.deleteTag(id.value)
    toast.add({
      title: 'Success',
...
onMounted(async () => {
  loading.value = true
  try {
    item.value = await tagsStore.fetchTag(id.value)
  } catch (error: any) {
    toast.add({
      title: 'Error',
